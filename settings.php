    ...
    <div class="add-edit-container" style="margin-bottom:20px;">
        ...
        <form action="settings.php?userId=<?= $userId ?>" method="POST">
            ...
            <div class="form-group">
                ...
                <small>wget <?= $projectPath ?>/ocr_reader.php?token=<?= urlencode($user['api_token']) ?></small><br>
                <small>wget <?= $projectPath ?>/ocr_remover.php?token=<?= urlencode($user['api_token']) ?></small>
            </div>
            <div class="form-group">
